...
table.insert(components.left.active, {
  ...
  hl = { fg = 'red' },
  right_sep = ' ',
  enabled = function() return vim.bo.readonly end
})
...
table.insert(components.left.active, {
  provider = function()
    ...
    if file_name == '' then file_name = '[unnamed]' end
...
table.insert(components.right.active, {
  ...
  enabled = function() return has_width_gt(30) end,
  hl = {
    fg = 'green',
  },
  icon = ' +',
...
table.insert(components.right.active, {
  ...
  enabled = function ()
    if vim.b.gitsigns_status_dict and has_width_gt(50) then return true end
  end,
  hl = {
    fg = 'cyan',
    style = 'bold'
...
table.insert(components.right.active, {
  provider = 'position',
  enabled = function() return has_width_gt(25) end,
  hl = {
    fg = 'light_gray',
  },
  left_sep = ' ',
...
table.insert(components.right.active, {
  ...
  left_sep = function()
    local val = {}
    if has_width_gt(25) then val.str = '' else val.str = ' ' end
